...
namespace LudoLib.Utilities
{
	public class FourPlayerBoard : IBoard
	{
		...
		public bool PlayersRanked
		{
			get
			{
				return !Players.Where(player => player.CanPlay == true)
					.Any();
			}
		}

		public FourPlayerBoard(IDice dice, IList<IPlayer> players, IDictionary<SquareSpot, List<IPiece>> piecesAtSquare)
		{
			Dice = dice;
			Players = players;
			PiecesAtSquare = piecesAtSquare;
		}
		...
		{
			if (!PiecesAtSquare.ContainsKey(selectedSpot))
			{
				return null;
			}

			return PiecesAtSquare[selectedSpot].Where(piece => piece.Color == pieceType)
				.Select(piece => piece)
				.ToList();
		}
		...
		{
			if (!movingPiece.CurrentSpot.HasValue || IsSafeSpot(othersSpot))
			{
				return;
			}
